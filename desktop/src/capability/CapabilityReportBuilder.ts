...
export class CapabilityReportBuilder {
  static buildMarkdown(snapshot: CapabilitySnapshot): string {
    ...
    const categoryOrder: TechnologyEntry['category'][] = ['language', 'framework', 'tool', 'platform', 'service', 'pattern'];
    ...
    for (const cat of categoryOrder) {
      ...
      for (const t of [...entries].sort((a, b) => b.exposureCount - a.exposureCount)) {
        const ws = t.workspaceIds.length;
        lines.push(
          `- **${t.name}** — ${t.exposureCount} mention${t.exposureCount !== 1 ? 's' : ''}` +
          ` across ${ws} workspace${ws !== 1 ? 's' : ''}` +
    ...
    if (wm.totalWorkflows === 0) {
      lines.push('No workflows defined yet.');
    } else {
      lines.push(
        `- **${wm.totalWorkflows}** workflow${wm.totalWorkflows !== 1 ? 's' : ''} defined`,
        `- **${wm.totalRuns}** total run${wm.totalRuns !== 1 ? 's' : ''}`,
        `- **${formatPct(wm.successRate)}** success rate`,
        `- Avg **${wm.avgStagesPerWorkflow}** stages / workflow`,
        `- Avg **${wm.avgStepsPerWorkflow}** steps / workflow`,
      );
      if (wm.mostRunWorkflowName) lines.push(`- Most-run playbook: **${wm.mostRunWorkflowName}**`);
    }
...
function groupByCategory(entries: TechnologyEntry[]): Partial<Record<TechnologyEntry['category'], TechnologyEntry[]>> {
  const out: Partial<Record<TechnologyEntry['category'], TechnologyEntry[]>> = {};
  for (const e of entries) (out[e.category] ??= []).push(e);
  return out;
}

function formatPct(ratio: number): string { return `${Math.round(ratio * 100)}%`; }
