    ...
    try {
      ...
      if (result.success) {
        ...
        if (this.db.version < 5) {
          const rawDb = this.db as Record<string, unknown>;
          if (!Array.isArray(rawDb['workflows'])) rawDb['workflows'] = [];
          if (!Array.isArray(rawDb['technologies'])) rawDb['technologies'] = [];
          if (!Array.isArray(rawDb['capabilitySnapshots'])) rawDb['capabilitySnapshots'] = [];
