...
export class WorkflowMetricsCalculator {
  static compute(workflows: Workflow[]): WorkflowMaturity {
    ...
    const completedRuns = workflows.reduce(
      (sum, w) => sum + w.runs.filter(r => r.status === 'completed').length,
      0,
    );
    const successRate = totalRuns > 0 ? completedRuns / totalRuns : 0;

    const avgStagesPerWorkflow =
      workflows.reduce((sum, w) => sum + w.stages.length, 0) / workflows.length;

    const avgStepsPerWorkflow =
      workflows.reduce(
        (sum, w) => sum + w.stages.reduce((s, stage) => s + stage.steps.length, 0),
        0,
      ) / workflows.length;
    ...
    for (const w of workflows) {
      if (w.runs.length > maxRuns) {
        maxRuns = w.runs.length;
        mostRunWorkflow = w;
      }
    }
